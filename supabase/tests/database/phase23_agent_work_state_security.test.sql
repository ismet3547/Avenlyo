...
select has_function(
  ...
  array['uuid'],
  'Phase 23 stable trusted message work-state read model exists'
);
select extensions.ok(
  has_function_privilege('service_role', 'public.get_message_agent_work_state(uuid)', 'EXECUTE'),
  'trusted backend can read stable message agent work state'
);
select extensions.ok(
...
select has_function(
  'public',
  'get_message_agent_work_state_v2',
  array['uuid'],
  'Phase 23 current runtime work-state exposes retry-visible provider review truth'
);
select extensions.ok(
  has_function_privilege('service_role', 'public.get_message_agent_work_state_v2(uuid)', 'EXECUTE')
  and not has_function_privilege('authenticated', 'public.get_message_agent_work_state_v2(uuid)', 'EXECUTE')
  and not has_function_privilege('anon', 'public.get_message_agent_work_state_v2(uuid)', 'EXECUTE'),
  'only the trusted backend can read the richer retry-safe work-state contract'
);
