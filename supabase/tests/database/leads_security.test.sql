...
select extensions.ok((select pg_temp.error_matches($sql$ update public.leads set status = 'converted' where conversation_id = 'e1600000-0000-0000-0000-000000000001' $sql$, '42501', 'permission denied.*')), 'authenticated members cannot directly mutate lead state');
select extensions.ok((select pg_temp.error_matches($sql$ select * from public.capture_conversation_lead('e1800000-0000-0000-0000-000000000001', 'forged', 'wellness', 'routine', 'appointment', null, '{}', 'qualified') $sql$, '42501', 'permission denied.*')), 'authenticated clients cannot execute service-only lead capture');
select extensions.ok((select pg_temp.error_matches($sql$ select public.convert_booking_lead('e1e00000-0000-0000-0000-000000000001', 'e1f00000-0000-0000-0000-000000000001') $sql$, '42501', 'permission denied.*')), 'authenticated clients cannot execute service-only conversion');
reset role;
...
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.leads (organization_id, location_id, contact_id, status, details) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1210000-0000-0000-0000-000000000001', 'lost', '{}') $sql$, '23503', 'foreign key constraint.*')), 'lead cannot reference a contact from another location');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.leads (organization_id, location_id, conversation_id, status, details) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1700000-0000-0000-0000-000000000001', 'lost', '{}') $sql$, '23503', 'foreign key constraint.*')), 'lead cannot reference a conversation from another location');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.leads (organization_id, location_id, conversation_id, last_captured_message_id, status, details) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1600000-0000-0000-0000-000000000001', 'e1900000-0000-0000-0000-000000000001', 'lost', '{}') $sql$, '23503', 'foreign key constraint.*')), 'lead cannot reference a captured message from another location');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.leads (organization_id, location_id, conversation_id, conversion_appointment_id, status, details) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1600000-0000-0000-0000-000000000001', 'e1f20000-0000-0000-0000-000000000001', 'lost', '{}') $sql$, '23503', 'foreign key constraint.*')), 'lead cannot reference a conversion appointment from another location');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.leads (organization_id, location_id, conversation_id, conversion_appointment_id, status, details) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1600000-0000-0000-0000-000000000001', 'e2f20000-0000-0000-0000-000000000001', 'lost', '{}') $sql$, '23503', 'foreign key constraint.*')), 'lead cannot reference a conversion appointment from another organization');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.lead_capture_tool_calls (organization_id, location_id, conversation_id, inbound_message_id, tool_call_id, lead_id, result_state) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1600000-0000-0000-0000-000000000001', 'e1830000-0000-0000-0000-000000000001', 'mismatched-message', (select id from public.leads where conversation_id = 'e1600000-0000-0000-0000-000000000001'), 'qualified') $sql$, '23503', 'foreign key constraint.*')), 'tool call cannot bind a message from another conversation');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.lead_capture_tool_calls (organization_id, location_id, conversation_id, inbound_message_id, tool_call_id, lead_id, result_state) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1600000-0000-0000-0000-000000000001', 'e1800000-0000-0000-0000-000000000001', 'mismatched-lead', (select id from public.leads where conversation_id = 'e1630000-0000-0000-0000-000000000001'), 'qualified') $sql$, '23503', 'foreign key constraint.*')), 'tool call cannot bind a lead from another conversation');
select extensions.ok((select pg_temp.error_matches($sql$ insert into public.lead_capture_tool_calls (organization_id, location_id, conversation_id, inbound_message_id, tool_call_id, lead_id, result_state) values ('e1000000-0000-0000-0000-000000000001', 'e1100000-0000-0000-0000-000000000001', 'e1700000-0000-0000-0000-000000000001', 'e1800000-0000-0000-0000-000000000001', 'mismatched-conversation', (select id from public.leads where conversation_id = 'e1600000-0000-0000-0000-000000000001'), 'qualified') $sql$, '23503', 'foreign key constraint.*')), 'tool call cannot bind a conversation from another location');
