...
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'manage_existing_subscription', 'unpaid blocks new Checkout');
reset role;
update public.billing_subscriptions set stripe_status = 'paused' where stripe_subscription_id = 'sub_guard';
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', 'b0000000-0000-0000-0000-000000000003', true);
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'manage_existing_subscription', 'paused blocks new Checkout');
reset role;
update public.billing_subscriptions set stripe_status = 'incomplete' where stripe_subscription_id = 'sub_guard';
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', 'b0000000-0000-0000-0000-000000000003', true);
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'manage_existing_subscription', 'incomplete blocks new Checkout');
reset role;
update public.billing_subscriptions set stripe_status = 'past_due' where stripe_subscription_id = 'sub_guard';
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', 'b0000000-0000-0000-0000-000000000003', true);
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'manage_existing_subscription', 'past_due blocks new Checkout');
reset role;
update public.billing_subscriptions set stripe_status = 'active' where stripe_subscription_id = 'sub_guard';
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', 'b0000000-0000-0000-0000-000000000003', true);
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'manage_existing_subscription', 'active blocks new Checkout');
reset role;
update public.billing_subscriptions set stripe_status = 'canceled' where stripe_subscription_id = 'sub_guard';
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', 'b0000000-0000-0000-0000-000000000003', true);
select extensions.is((select action from public.begin_my_billing_checkout('core')), 'create_checkout', 'canceled history permits a new Checkout');
reset role;
