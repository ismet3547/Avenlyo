...
select extensions.lives_ok(
  $$ select * from public.request_inbound_voice_handoff('rtc_voice_a_one', 'tool_1', 'Caller requested a person.', 'normal') $$,
  'service backend creates a same-tenant voice handoff'
);
select extensions.is(
  (select count(*)::integer from public.handoffs where call_id = '61600000-0000-0000-0000-000000000001'),
  1,
  'voice handoff remains tied to its trusted call and conversation'
);
