...
select extensions.set_eq(
  ...
  $q$ values
      ...
      ('get_my_ai_usage_summary(target_location_id uuid, lookback_days integer)'),
      ...
      ('record_agent_test_ai_usage(target_run_id uuid, target_model_tier text, target_route_reason text, target_input_tokens integer, target_cached_input_tokens integer, target_output_tokens integer, target_estimated_cost_microusd bigint, target_pricing_version text)'),
...
select extensions.is_empty(
  $q$ select proc
      from unnest(array[
        ...
        'public.record_message_ai_usage(uuid,text,text,text,integer,integer,integer,bigint,text)',
