...
language plpgsql security definer set search_path = '' as $$
declare conversation_row public.conversations%rowtype; channel_row public.channels%rowtype; trusted_inbound public.messages%rowtype;
  sms_route public.phone_numbers%rowtype; saved_message_id uuid; contact_opted_out boolean;
begin
  ...
  if channel_row.channel_type = 'sms' then
    select * into sms_route from public.phone_numbers phone where phone.organization_id = conversation_row.organization_id
      and phone.location_id = conversation_row.location_id and phone.id = conversation_row.transport_phone_number_id
      and phone.status = 'active' and phone.sms_enabled;
    if conversation_row.status <> 'open' or sms_route.id is null then raise exception using errcode = '42501', message = 'SMS route is unavailable'; end if;
    select * into trusted_inbound from public.messages inbound where inbound.organization_id = conversation_row.organization_id
      and inbound.location_id = conversation_row.location_id and inbound.conversation_id = conversation_row.id
      and inbound.direction = 'inbound' and inbound.source_channel = 'sms' and inbound.author_type = 'customer'
      and inbound.transport_sender_e164 is not null
      order by inbound.created_at desc, inbound.id desc limit 1;
    if trusted_inbound.id is null then raise exception using errcode = '42501', message = 'SMS transport identity is unavailable'; end if;
    ...
    if contact_opted_out then raise exception using errcode = '42501', message = 'SMS contact has opted out'; end if;
  end if;
  insert into public.messages (organization_id, location_id, conversation_id, contact_id, direction, message_type, body, metadata, source_channel, author_type, sent_by_user_id, in_reply_to_message_id, sent_at)
  values (conversation_row.organization_id, conversation_row.location_id, conversation_row.id, conversation_row.contact_id, 'outbound', 'text', btrim(target_body),
    jsonb_build_object('transport', channel_row.channel_type), channel_row.channel_type, 'human', auth.uid(), trusted_inbound.id, now()) returning id into saved_message_id;
  if channel_row.channel_type = 'sms' then
