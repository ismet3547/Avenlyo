...
begin
  ...
  update public.voice_sms_followup_consent_intents as pending set status = 'expired', updated_at = now() where pending.organization_id = call_row.organization_id and pending.call_id = call_row.id and pending.purpose = 'lead_followup' and pending.status = 'awaiting_confirmation' and pending.expires_at <= now();
  select * into intent from public.voice_sms_followup_consent_intents as pending where pending.organization_id = call_row.organization_id and pending.call_id = call_row.id and pending.purpose = 'lead_followup' and pending.status = 'awaiting_confirmation' and pending.expires_at > now();
...
begin
  ...
  on conflict (organization_id,location_id) do update set lead_followup_enabled=excluded.lead_followup_enabled,delay_minutes=excluded.delay_minutes,quiet_hours_start=excluded.quiet_hours_start,quiet_hours_end=excluded.quiet_hours_end,business_hours_only=excluded.business_hours_only,sender_phone_number_id=excluded.sender_phone_number_id,automation_acknowledged_at=case when excluded.lead_followup_enabled then coalesce(s.automation_acknowledged_at,excluded.automation_acknowledged_at) else null end,automation_acknowledged_by=case when excluded.lead_followup_enabled then coalesce(s.automation_acknowledged_by,excluded.automation_acknowledged_by) else null end,updated_at=now() returning id into settings_id;
