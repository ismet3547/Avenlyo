...
export async function resolveOwnedAnimal(
  ...
  const payload = await client.get('/v2/animal', {
    'filter[contact.uid][eq]': customer.key,
  });
  ...
  for (const item of items(payload)) {
    ...
    if (boolean(animal.active) === false || boolean(animal.deceased) === true) continue;
    const owner = string(animal.contact_uid ?? animal.contactUid ?? animal.owner_uid ?? animal.ownerUid);
    const name = string(animal.name);
    const key = string(animal.uid ?? animal.id);
    if (!owner || owner !== customer.key || !name || !key || normalizedName(name) !== desiredName) continue;
