...
export const ezyVetSchedulingRoutes: FastifyPluginCallback = (app, _options, done) => {
  app.post(
    '/v1/scheduling/ezyvet/:locationId/connect',
    ...
    async (request, reply) => {
      ...
      if (!locationId || !validUuid(locationId) || !body) {
        return reply
          .code(400)
          .send({ code: 'INVALID_REQUEST', message: 'Connection details are invalid.' });
      }
  ...
  app.post(
    '/v1/scheduling/ezyvet/:locationId/catalog-sync',
    { preHandler: app.authenticate },
    async (request, reply) => {
      const locationId = (request.params as { locationId?: string }).locationId;
      const service = requireService();
  ...
  app.post(
    '/v1/scheduling/ezyvet/:locationId/disconnect',
    { preHandler: app.authenticate },
    async (request, reply) => {
      const locationId = (request.params as { locationId?: string }).locationId;
      const service = requireService();
